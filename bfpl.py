
#!/usr/bin/env python3
# -*- coding: GBK -*-
"""
BFPL (Brainfuck Pointer Language) 图灵机模拟器
特性：
- 多图灵机纸带，无限长
- 指针操作 (+ - R L)
- 输入/输出 (I O)
- 整数变量与表达式求值 (D/E)
- 标签与条件跳转 (J JZ JNZ)
- 行注释 (#)
- 调试模式 (-d)
- 静默输出模式 (-q) 仅输出 O/N 指令的结果
- [] 循环 (支持嵌套)
- FileInput Path / FileOutput Path (输入/输出重定向)
- O/N 支持 Now/Last 后缀（默认 Last）
- I 支持 Num/Char 后缀（默认 Char）
"""

import re
import sys
from collections import defaultdict
import argparse

# ---------------------------- 安全表达式求值 ----------------------------
class SafeEvaluator:
    """安全的算术表达式求值器（仅支持整数 + - * / % 和括号）"""
    @staticmethod
    def evaluate(expr, variables):
        def repl_var(m):
            name = m.group(0)
            if name not in variables:
                raise NameError(f"未定义的变量: {name}")
            return str(variables[name])
        expr_with_values = re.sub(r'\b[a-zA-Z_][a-zA-Z0-9_]*\b', repl_var, expr)
        if not re.match(r'^[\d\s\+\-\*\/\%\(\)]+$', expr_with_values):
            raise ValueError(f"表达式包含非法字符: {expr}")
        try:
            return eval(expr_with_values, {"__builtins__": None}, {})
        except ZeroDivisionError:
            raise ValueError("除零错误")
        except Exception as e:
            raise ValueError(f"表达式计算失败: {expr} -> {e}")

# ---------------------------- 图灵机纸带 ----------------------------
class TuringMachine:
    def __init__(self, name):
        self.name = name
        self.tape = defaultdict(int)

    def read(self, pos):
        return self.tape[pos]

    def write(self, pos, value):
        self.tape[pos] = value

    def __repr__(self):
        items = sorted(self.tape.items())
        non_zero = [f"{p}:{v}" for p, v in items if v != 0]
        if not non_zero:
            return f"TuringMachine({self.name}): (all zeros)"
        return f"TuringMachine({self.name}): {{{', '.join(non_zero)}}}"

# ---------------------------- 指针 ----------------------------
class Pointer:
    def __init__(self, name, machine, pos=0):
        self.name = name
        self.machine = machine
        self.pos = pos

    def inc(self, delta=1):
        v = self.machine.read(self.pos)
        self.machine.write(self.pos, v + delta)

    def dec(self, delta=1):
        v = self.machine.read(self.pos)
        self.machine.write(self.pos, v - delta)

    def right(self, steps=1):
        self.pos += steps

    def left(self, steps=1):
        self.pos -= steps

    def read_char(self):
        return chr(self.machine.read(self.pos))

    def write_char(self, ch):
        self.machine.write(self.pos, ord(ch))

    def __repr__(self):
        return f"Pointer({self.name} @ {self.machine.name}[{self.pos}] = {self.machine.read(self.pos)})"

# ---------------------------- 模拟器核心 ----------------------------
class TuringSimulator:
    def __init__(self, debug=False, quiet=False):
        self.variables = {}
        self.machines = {}
        self.pointers = {}
        self.labels = {}
        self.statements = []
        self.ip = 0
        self.debug = debug
        self.quiet = quiet          # 静默模式：只输出 N 和 O 指令的内容
        self.running = True
        self.bracket_pairs = {}
        self.output_chars = []      # 统一收集 Last 模式的输出字符/字符串
        self.step_count = 0
        self.max_steps = 1000000    # 防止死循环

        # ---- 文件重定向支持 ----
        self.input_file = None      # 当前输入文件对象
        self.output_file = None     # 当前输出文件对象

    def execute(self, code):
        # 去除注释
        code = re.sub(r'#.*$', '', code, flags=re.MULTILINE)
        raw_lines = [s.strip() for s in code.split(';') if s.strip()]

        self.statements = []
        self.labels.clear()
        for line in raw_lines:
            if line.startswith(':'):
                label = line[1:].strip()
                if label in self.labels:
                    raise SyntaxError(f"重复的标签: {label}")
                self.labels[label] = len(self.statements)
            else:
                self.statements.append(line)

        # 括号匹配
        self.bracket_pairs = {}
        stack = []
        for idx, stmt in enumerate(self.statements):
            parts = stmt.split(maxsplit=1)
            if not parts:
                continue
            cmd = parts[0]
            if cmd == '[':
                stack.append(idx)
            elif cmd == ']':
                if not stack:
                    raise SyntaxError(f"未匹配的 ']' 在索引 {idx}")
                left = stack.pop()
                self.bracket_pairs[left] = idx
                self.bracket_pairs[idx] = left
        if stack:
            raise SyntaxError(f"未匹配的 '[' 在索引 {stack[0]}")

        self.ip = 0
        self.running = True
        self.step_count = 0

        while 0 <= self.ip < len(self.statements) and self.running:
            self.step_count += 1
            if self.step_count > self.max_steps:
                raise RuntimeError(f"执行步数超过上限 {self.max_steps}，可能陷入死循环")

            stmt = self.statements[self.ip]
            if self.debug and not self.quiet:
                print(f"[DEBUG] IP={self.ip}: {stmt}")

            jumped = self._exec_stmt(stmt)
            if not self.running:
                break
            if not jumped:
                self.ip += 1

    def _get_value(self, arg):
        if re.match(r'^-?\d+$', arg):
            return int(arg)
        return SafeEvaluator.evaluate(arg, self.variables)

    def _exec_stmt(self, stmt):
        parts = stmt.split(maxsplit=4)   # 扩展到4个参数以支持 I/O 新参数
        if not parts:
            return False
        cmd = parts[0]

        # ---------- 方括号循环 ----------
        if cmd == '[':
            if len(parts) < 2:
                raise SyntaxError("[ 需要指针名")
            ptr_name = parts[1]
            if ptr_name not in self.pointers:
                raise ValueError(f"指针 {ptr_name} 不存在")
            ptr = self.pointers[ptr_name]
            if ptr.machine.read(ptr.pos) == 0:
                if self.ip not in self.bracket_pairs:
                    raise RuntimeError("括号匹配错误")
                self.ip = self.bracket_pairs[self.ip] + 1
                if self.debug and not self.quiet:
                    print(f"[ 条件为0，跳过循环体，跳转到 {self.ip}")
                return True
            else:
                if self.debug and not self.quiet:
                    print(f"[ 条件非0，进入循环体")
                return False

        elif cmd == ']':
            if self.ip not in self.bracket_pairs:
                raise RuntimeError("括号匹配错误")
            self.ip = self.bracket_pairs[self.ip]
            if self.debug and not self.quiet:
                print(f"] 跳转到 {self.ip}")
            return True

        # ---------- 其余指令（转为大写） ----------
        cmd_upper = cmd.upper()

        # D name = expr   (定义变量)
        if cmd_upper == 'D':
            if len(parts) < 3:
                raise SyntaxError(f"D 需要两个参数: {stmt}")
            var_name = parts[1]
            value_expr = parts[2]
            self.variables[var_name] = self._get_value(value_expr)
            if self.debug and not self.quiet:
                print(f"定义变量 {var_name} = {self.variables[var_name]}")
            return False

        # N var [Last|Now]   (打印变量值)
        elif cmd_upper == 'N':
            if len(parts) < 2:
                raise SyntaxError(f"N 需要一个变量名: {stmt}")
            var_name = parts[1]
            value = self.variables.get(var_name, 0)

            # 解析输出时机参数，默认 Last
            output_timing = 'Last'
            if len(parts) >= 3:
                timing = parts[2].upper()
                if timing in ('LAST', 'NOW'):
                    output_timing = timing
                else:
                    raise SyntaxError(
                        f"N 指令的第三个参数必须是 Last 或 Now，得到: {parts[2]}"
                    )

            value_str = str(value)

            if output_timing == 'NOW':
                # 立即输出
                if self.output_file is not None:
                    self.output_file.write(value_str)
                else:
                    sys.stdout.write(value_str)
                    sys.stdout.flush()
            else:  # 'LAST'
                # 收集到缓冲区，最后统一输出
                self.output_chars.append(value_str)

            if self.debug and not self.quiet:
                print(f"N: {var_name} = {value} [{output_timing}]")
            return False

        # M name   (创建图灵机)
        elif cmd_upper == 'M':
            if len(parts) < 2:
                raise SyntaxError(f"M 需要一个参数: {stmt}")
            tur_name = parts[1]
            if tur_name in self.machines:
                if self.debug and not self.quiet:
                    print(f"警告: 图灵机 {tur_name} 已存在，将被覆盖")
            self.machines[tur_name] = TuringMachine(tur_name)
            if self.debug and not self.quiet:
                print(f"创建图灵机: {tur_name}")
            return False

        # S ptr_name machine_name   (定义指针)
        elif cmd_upper == 'S':
            if len(parts) < 3:
                raise SyntaxError(f"S 需要两个参数: {stmt}")
            pos_name = parts[1]
            tur_name = parts[2]
            if tur_name not in self.machines:
                raise ValueError(f"图灵机 {tur_name} 不存在")
            self.pointers[pos_name] = Pointer(pos_name, self.machines[tur_name], 0)
            if self.debug and not self.quiet:
                print(f"定义指针 {pos_name} 指向 {tur_name}[0]")
            return False

        # + ptr [delta]
        elif cmd_upper == '+':
            if len(parts) < 2:
                raise SyntaxError(f"+ 需要一个指针名: {stmt}")
            pos_name = parts[1]
            if pos_name not in self.pointers:
                raise ValueError(f"指针 {pos_name} 不存在")
            delta = 1
            if len(parts) >= 3:
                delta = self._get_value(parts[2])
            self.pointers[pos_name].inc(delta)
            if self.debug and not self.quiet:
                print(f"{pos_name} +{delta} -> {self.pointers[pos_name]}")
            return False

        # - ptr [delta]
        elif cmd_upper == '-':
            if len(parts) < 2:
                raise SyntaxError(f"- 需要一个指针名: {stmt}")
            pos_name = parts[1]
            if pos_name not in self.pointers:
                raise ValueError(f"指针 {pos_name} 不存在")
            delta = 1
            if len(parts) >= 3:
                delta = self._get_value(parts[2])
            self.pointers[pos_name].dec(delta)
            if self.debug and not self.quiet:
                print(f"{pos_name} -{delta} -> {self.pointers[pos_name]}")
            return False

        # R ptr [steps]
        elif cmd_upper == 'R':
            if len(parts) < 2:
                raise SyntaxError(f"R 需要一个指针名: {stmt}")
            pos_name = parts[1]
            if pos_name not in self.pointers:
                raise ValueError(f"指针 {pos_name} 不存在")
            steps = 1
            if len(parts) >= 3:
                steps = self._get_value(parts[2])
            self.pointers[pos_name].right(steps)
            if self.debug and not self.quiet:
                print(f"{pos_name} 右移 {steps} -> {self.pointers[pos_name]}")
            return False

        # L ptr [steps]
        elif cmd_upper == 'L':
            if len(parts) < 2:
                raise SyntaxError(f"L 需要一个指针名: {stmt}")
            pos_name = parts[1]
            if pos_name not in self.pointers:
                raise ValueError(f"指针 {pos_name} 不存在")
            steps = 1
            if len(parts) >= 3:
                steps = self._get_value(parts[2])
            self.pointers[pos_name].left(steps)
            if self.debug and not self.quiet:
                print(f"{pos_name} 左移 {steps} -> {self.pointers[pos_name]}")
            return False

        # O ptr [Last|Now]   (输出字符)
        elif cmd_upper == 'O':
            if len(parts) < 2:
                raise SyntaxError(f"O 需要一个指针名: {stmt}")
            pos_name = parts[1]
            if pos_name not in self.pointers:
                raise ValueError(f"指针 {pos_name} 不存在")

            # 解析输出时机参数，默认 Last
            output_timing = 'Last'
            if len(parts) >= 3:
                timing = parts[2].upper()
                if timing in ('LAST', 'NOW'):
                    output_timing = timing
                else:
                    raise SyntaxError(
                        f"O 指令的第三个参数必须是 Last 或 Now，得到: {parts[2]}"
                    )

            ch = self.pointers[pos_name].read_char()

            if output_timing == 'NOW':
                # 立即输出
                if self.output_file is not None:
                    self.output_file.write(ch)
                else:
                    sys.stdout.write(ch)
                    sys.stdout.flush()
            else:  # 'LAST'
                # 收集到缓冲区，最后统一输出
                self.output_chars.append(ch)

            if self.debug and not self.quiet:
                print(f"输出: '{ch}' (ASCII {ord(ch)}) [{output_timing}]")
            return False

        # I ptr [Char|Num]   (输入)
        elif cmd_upper == 'I':
            if len(parts) < 2:
                raise SyntaxError(f"I 需要一个指针名: {stmt}")
            pos_name = parts[1]
            if pos_name not in self.pointers:
                raise ValueError(f"指针 {pos_name} 不存在")

            # 解析输入类型参数，默认 Char
            input_type = 'Char'
            if len(parts) >= 3:
                itype = parts[2].upper()
                if itype in ('CHAR', 'NUM'):
                    input_type = itype
                else:
                    raise SyntaxError(
                        f"I 指令的第三个参数必须是 Char 或 Num，得到: {parts[2]}"
                    )

            if input_type == 'CHAR':
                # 字符输入
                if self.input_file is not None:
                    ch = self.input_file.read(1)
                    if ch == '':
                        ch = '\x00'      # EOF 视为空字符
                else:
                    if sys.stdin.isatty():
                        sys.stdout.write("> ")
                        sys.stdout.flush()
                    user_input = sys.stdin.readline().strip()
                    if user_input:
                        ch = user_input[0]
                    else:
                        ch = '\x00'

                self.pointers[pos_name].write_char(ch)
                if self.debug and not self.quiet:
                    print(f"输入字符 '{ch}' 写入 {pos_name}")

            else:  # 'NUM'
                # 数字输入（读取一行，以换行结尾）
                if self.input_file is not None:
                    line = self.input_file.readline()
                    if line == '':
                        num = 0          # EOF 视为 0
                    else:
                        try:
                            num = int(line.strip())
                        except ValueError:
                            num = 0
                else:
                    if sys.stdin.isatty():
                        sys.stdout.write("请输入数字: ")
                        sys.stdout.flush()
                    line = sys.stdin.readline()
                    try:
                        num = int(line.strip())
                    except ValueError:
                        num = 0

                self.pointers[pos_name].machine.write(
                    self.pointers[pos_name].pos, num
                )
                if self.debug and not self.quiet:
                    print(f"输入数字 {num} 写入 {pos_name}")

            return False

        # J label   (无条件跳转)
        elif cmd_upper == 'J':
            if len(parts) < 2:
                raise SyntaxError(f"J 需要一个标签: {stmt}")
            label = parts[1]
            if label not in self.labels:
                raise ValueError(f"标签 {label} 不存在")
            self.ip = self.labels[label]
            if self.debug and not self.quiet:
                print(f"跳转到 {label} (IP={self.ip})")
            return True

        # JZ var label   (变量为0跳转)
        elif cmd_upper == 'JZ':
            if len(parts) < 3:
                raise SyntaxError(f"JZ 需要两个参数: 变量 标签")
            var_name = parts[1]
            label = parts[2]
            if label not in self.labels:
                raise ValueError(f"标签 {label} 不存在")
            val = self.variables.get(var_name, 0)
            if val == 0:
                self.ip = self.labels[label]
                if self.debug and not self.quiet:
                    print(f"{var_name} == 0 -> 跳转到 {label}")
                return True
            else:
                if self.debug and not self.quiet:
                    print(f"{var_name} = {val} != 0, 不跳转")
                return False

        # JNZ var label   (变量非0跳转)
        elif cmd_upper == 'JNZ':
            if len(parts) < 3:
                raise SyntaxError(f"JNZ 需要两个参数: 变量 标签")
            var_name = parts[1]
            label = parts[2]
            if label not in self.labels:
                raise ValueError(f"标签 {label} 不存在")
            val = self.variables.get(var_name, 0)
            if val != 0:
                self.ip = self.labels[label]
                if self.debug and not self.quiet:
                    print(f"{var_name} != 0 -> 跳转到 {label}")
                return True
            else:
                if self.debug and not self.quiet:
                    print(f"{var_name} = 0, 不跳转")
                return False

        # H   (停机)
        elif cmd_upper == 'H':
            self.running = False
            if self.debug and not self.quiet:
                print("停机")
            return False

        # E var = expr   (赋值，与 D 相同)
        elif cmd_upper == 'E':
            if len(parts) < 3:
                raise SyntaxError(f"E 需要两个参数: {stmt}")
            var_name = parts[1]
            value_expr = parts[2]
            self.variables[var_name] = self._get_value(value_expr)
            if self.debug and not self.quiet:
                print(f"赋值 {var_name} = {self.variables[var_name]}")
            return False

        # FileInput Path / FileOutput Path
        elif cmd_upper == 'FILEINPUT':
            if len(parts) < 2:
                raise SyntaxError(f"FileInput 需要一个路径参数: {stmt}")
            path = parts[1]
            if self.input_file is not None:
                self.input_file.close()
            try:
                self.input_file = open(path, 'r', encoding='utf-8')
            except Exception as e:
                raise RuntimeError(f"无法打开输入文件 {path}: {e}")
            if self.debug and not self.quiet:
                print(f"输入重定向到文件: {path}")
            return False

        elif cmd_upper == 'FILEOUTPUT':
            if len(parts) < 2:
                raise SyntaxError(f"FileOutput 需要一个路径参数: {stmt}")
            path = parts[1]
            if self.output_file is not None:
                self.output_file.close()
            try:
                self.output_file = open(path, 'w', encoding='utf-8')
            except Exception as e:
                raise RuntimeError(f"无法打开输出文件 {path}: {e}")
            if self.debug and not self.quiet:
                print(f"输出重定向到文件: {path}")
            return False

        else:
            raise SyntaxError(f"未知指令: {cmd}")

# ---------------------------- 主程序入口 ----------------------------
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="BFPL 图灵机模拟器",
        epilog="示例: python bfpl.py demo.bfpl -d"
    )
    parser.add_argument('filename', help='源文件 (.bfpl)')
    parser.add_argument('-d', '--debug', action='store_true', help='开启调试模式')
    parser.add_argument('-q', '--quiet', action='store_true',
                        help='静默模式：只输出 N 和 O 指令的结果，不显示调试信息和最终状态')
    args = parser.parse_args()

    # 读取文件（优先 UTF-8，失败则尝试 GBK）
    try:
        with open(args.filename, 'r', encoding='utf-8') as f:
            program = f.read()
    except UnicodeDecodeError:
        try:
            with open(args.filename, 'r', encoding='gbk') as f:
                program = f.read()
        except Exception as e:
            print(f"无法读取文件，编码错误: {e}")
            sys.exit(1)
    except FileNotFoundError:
        print(f"错误: 文件不存在 '{args.filename}'")
        sys.exit(1)

    sim = TuringSimulator(debug=args.debug, quiet=args.quiet)
    try:
        sim.execute(program)
    except Exception as e:
        print(f"\n运行时错误: {e}")
        sys.exit(1)
    finally:
        # ---- 处理 Last 缓冲区的输出 ----
        if sim.output_chars:
            if sim.output_file is not None:
                sim.output_file.write(''.join(sim.output_chars))
                sim.output_file.close()
            else:
                if not args.quiet:
                    print("\n=== 最后输出 ===")
                print(''.join(sim.output_chars))

        # 关闭可能打开的输入文件
        if sim.input_file is not None:
            sim.input_file.close()

        # 如果输出文件仍存在但未关闭（比如没有输出内容），关闭它
        if sim.output_file is not None and not sim.output_file.closed:
            sim.output_file.close()

    # 非静默模式打印最终状态（只在没有发生运行时错误时输出）
    if not args.quiet:
        print("\n=== 最终状态 ===")
        for m in sim.machines.values():
            print(m)
        for p in sim.pointers.values():
            print(p)
